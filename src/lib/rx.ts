...
export function rxScoreBreakdown(
  ...
  if (isConfirmed(c, "phone"))
    items.push({ label: "Telefone confirmado na fonte", points: w.phone });
  if (isConfirmed(c, "whatsapp"))
    items.push({ label: "WhatsApp confirmado na fonte", points: w.whatsapp });
  ...
  if (isConfirmed(c, "instagram"))
    items.push({ label: "Instagram confirmado", points: w.instagram });
  if (isConfirmed(c, "linkedin")) items.push({ label: "LinkedIn confirmado", points: w.linkedin });
  if (isConfirmed(c, "independent_local"))
    items.push({
      label: "Empresa local independente (sem marca/rede na fonte)",
...
export const SEGMENT_PRESETS: { label: string; match: string[]; tags: string[] }[] = [
  {
    ...
    match: ["salao", "salão", "beleza", "cabelele"],
  ...
  { label: "Barbearia", match: ["barbear"], tags: ["shop=hairdresser", "shop=beauty"] },
  {
    ...
    tags: ["shop=beauty", "amenity=clinic"],
  },
  ...
  {
    label: "Lanchonete",
    match: ["lanchonete", "hamburg", "fast food"],
  ...
  { label: "Academia", match: ["academia", "fitness", "cross"], tags: ["leisure=fitness_centre"] },
  {
    label: "Pet shop",
    match: ["pet", "veterin"],
    tags: ["shop=pet", "shop=pet_grooming", "amenity=veterinary"],
  },
  { label: "Odontologia", match: ["dentista", "odonto"], tags: ["amenity=dentist"] },
  {
    label: "Clínica médica",
    match: ["clinica", "clínica", "medic", "saude", "saúde"],
  ...
  { label: "Imobiliária", match: ["imobili", "corretor"], tags: ["office=estate_agent"] },
  {
    ...
    match: ["oficina", "mecanic", "mecânic", "auto"],
  ...
  {
    label: "Escola / Curso",
    match: ["escola", "curso", "idioma"],
    tags: ["amenity=school", "amenity=language_school", "office=educational_institution"],
  },
  {
    label: "Mercado",
    match: ["mercado", "supermerc", "hortifr"],
...
export function candidateKeys(c: {
  ...
  const place = normalizeCompany(`${c.company ?? ""} ${c.neighborhood || c.address || ""}`.trim());
